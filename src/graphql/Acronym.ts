...
export const AcronymQuery = extendType({
    ...
    definition(t) {
        ...
        t.field('acronym', {
            ...
            resolve(parent, args, context) {
                return context.prisma.acronym.findUnique({ where: { id: args?.id as number | undefined, } })
            }
        })
    }
})
    ...
    definition(t) {
        ...
        t.field('patch', {
            ...
            async resolve(parent, args, context) {
                const originAcronym = await context.prisma.acronym.findUnique({ where: { id: args?.id as number | undefined, } })
                const updateAcronym = context.prisma.acronym.update({
                    where: {
                        id: args?.id,
                    },
                    data: {
                        acronym_form: args.acronym_form ? args.acronym_form : originAcronym?.acronym_form,
                        full_form: args.full_form ? args.full_form : originAcronym?.full_form,
                    },
                })
        ...
        t.nonNull.field('delete', {
            ...
            resolve(parent, args, context) {
                return context.prisma.acronym.delete({ where: { id: args?.id as number | undefined, } })
            }
        })
    }
})
